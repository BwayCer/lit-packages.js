import { viewModel, litViewModel, litViewModelUnsafeHTML } from './litViewModel.js';
...
class s$3 {
  ...
  get styleSheet() {
    ...
    return t$1 && void 0 === e && (n$4.set(this.cssText, e = new CSSStyleSheet()), e.replaceSync(this.cssText)), e;
  }
...
const e$2 = window.reactiveElementPolyfillSupport,
      r$1 = {
  toAttribute(t, i) {
    switch (i) {
      case Boolean:
...
n$3.finalized = !0, n$3.elementProperties = new Map(), n$3.elementStyles = [], n$3.shadowRootOptions = {
  mode: "open"
}, null == e$2 || e$2({
  ReactiveElement: n$3
...
const i$1 = globalThis.trustedTypes,
  ...
  for (let i = 0; i < o; i++) {
    ...
    r += d === c ? s + n$2 : $ >= 0 ? (l.push(o), s.slice(0, $) + "$lit$" + s.slice($) + e$1 + y) : s + e$1 + (-2 === $ ? (l.push(void 0), i) : y);
  }
...
let MyElement = _decorate([n('my-element')], function (_initialize, _LitElement) {
  ...
  return {
    ...
    d: [{
      ...
      decorators: [e({
        type: Object,
        reflect: true,
        attribute: 'data-interpolation-name'
      })],
      key: "interpolationName",
    ...
    `;
      ...
      value: function render() {
        let interpolations = {
          name: this.interpolationName
        };
        return p`
<pre><code class="tagCode">&lt;my-element data-lifecycle-count="${this._updateCount + 1}" data-interpolation-name="${this.interpolationName}"&gt;</code>
  touchRootElement: <button @click=${() => this.requestUpdate()}>requestUpdate</button>
  interpolations: { name: <input type="text" @change=${evt => this.interpolationName = evt.currentTarget.value} value=${this.interpolationName}></input> }
  text: <input class="interpolationNameInput" type="text" @change=${evt => viewModel.set('main.text', evt.currentTarget.value)} value=${litViewModel('main.text')}></input>
  state: "${litViewModel('main.text', {
          name: this.interpolationName
        })}"
  stateHTML: "${litViewModelUnsafeHTML('main.text', interpolations)}"
  text length: "${litViewModel('main.text', modelValue => modelValue.getValue(interpolations).toString().length)}"
...
viewModel.createData('main', {
  text: 'Hello <span style="background: #556b2f; color: #f0ffff;">{{name}}</span>'
});
